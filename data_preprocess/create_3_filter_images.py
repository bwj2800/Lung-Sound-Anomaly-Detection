...
def create_mel_raw(current_window, sample_rate, n_mels=128, f_min=50, f_max=4000, nfft=2048, hop=512, resz=1):
    ...
    S_low_filtered = (S_low_filtered-S_low_filtered.min()) / (S_low_filtered.max() - S_low_filtered.min())
    S_low_filtered *= 255
    img_low = cv2.applyColorMap(S_low_filtered.astype(np.uint8), cmapy.cmap('magma'))
    img_low = cv2.flip(img_low, 0)
    
    S_high_filtered = (S_high_filtered-S_high_filtered.min()) / (S_high_filtered.max() - S_high_filtered.min())
    S_high_filtered *= 255
    img_high = cv2.applyColorMap(S_high_filtered.astype(np.uint8), cmapy.cmap('magma'))
    img_high = cv2.flip(img_high, 0)
    
    S_band_filtered = (S_band_filtered-S_band_filtered.min()) / (S_band_filtered.max() - S_band_filtered.min())
    S_band_filtered *= 255
    img_band = cv2.applyColorMap(S_band_filtered.astype(np.uint8), cmapy.cmap('magma'))
    img_band = cv2.flip(img_band, 0)
 
 
    return img_low, img_high, img_band
...
for idx, file_name in enumerate(tqdm(filenames)):
...
for index in tqdm(range(len(audio_data))): #len(audio_data)
    ...
    img_low, img_high, img_band = create_mel_raw(audio, sample_rate, f_max= f_max, n_mels=n_mels, nfft=nfft, hop=hop, resz=1)
    img_low = cv2.cvtColor(img_low, cv2.COLOR_BGR2RGB)
    img_high = cv2.cvtColor(img_high, cv2.COLOR_BGR2RGB)
    img_band = cv2.cvtColor(img_band, cv2.COLOR_BGR2RGB)
    
    mel_img_low.append((img_low, label))
    mel_img_high.append((img_high, label))
    mel_img_band.append((img_band, label))

destination_dir = '../data_4gr/mel_image_3/'
low_dir = os.path.join(destination_dir, 'low')
high_dir = os.path.join(destination_dir, 'high')
band_dir = os.path.join(destination_dir, 'band')

if not os.path.exists(destination_dir):
    os.makedirs(destination_dir)
# Create the four folders for the labels
os.makedirs(os.path.join(low_dir,'normal'), exist_ok=True)
os.makedirs(os.path.join(low_dir,'crackle'), exist_ok=True)
os.makedirs(os.path.join(low_dir,'wheeze'), exist_ok=True)
os.makedirs(os.path.join(low_dir,'both'), exist_ok=True)
os.makedirs(os.path.join(high_dir,'normal'), exist_ok=True)
os.makedirs(os.path.join(high_dir,'crackle'), exist_ok=True)
os.makedirs(os.path.join(high_dir,'wheeze'), exist_ok=True)
os.makedirs(os.path.join(high_dir,'both'), exist_ok=True)
os.makedirs(os.path.join(band_dir,'normal'), exist_ok=True)
os.makedirs(os.path.join(band_dir,'crackle'), exist_ok=True)
os.makedirs(os.path.join(band_dir,'wheeze'), exist_ok=True)
os.makedirs(os.path.join(band_dir,'both'), exist_ok=True)

print('Saving images...')
for i in tqdm(range(len(mel_img_low))):
    input_data = mel_img_low[i][0]
    # print(input_data)
    labels = mel_img_low[i][1]
    # print(type(labels))
    
    if labels == 0: #1: abnormal, 0: normal
        cv2.imwrite(os.path.join(low_dir,'normal', 'image_'+str(i)+'.jpg'), cv2.cvtColor(input_data, cv2.COLOR_RGB2BGR))
    elif labels == 1:
        cv2.imwrite(os.path.join(low_dir,'crackle', 'image_'+str(i)+'.jpg'), cv2.cvtColor(input_data, cv2.COLOR_RGB2BGR))
    elif labels == 2:
        cv2.imwrite(os.path.join(low_dir,'wheeze', 'image_'+str(i)+'.jpg'), cv2.cvtColor(input_data, cv2.COLOR_RGB2BGR))
    else:
        cv2.imwrite(os.path.join(low_dir,'both', 'image_'+str(i)+'.jpg'), cv2.cvtColor(input_data, cv2.COLOR_RGB2BGR))

for i in tqdm(range(len(mel_img_high))):
    input_data = mel_img_high[i][0]
    # print(input_data)
    labels = mel_img_high[i][1]
    # print(type(labels))
    
    if labels == 0: #1: abnormal, 0: normal
        cv2.imwrite(os.path.join(high_dir,'normal', 'image_'+str(i)+'.jpg'), cv2.cvtColor(input_data, cv2.COLOR_RGB2BGR))
    elif labels == 1:
        cv2.imwrite(os.path.join(high_dir,'crackle', 'image_'+str(i)+'.jpg'), cv2.cvtColor(input_data, cv2.COLOR_RGB2BGR))
    elif labels == 2:
        cv2.imwrite(os.path.join(high_dir,'wheeze', 'image_'+str(i)+'.jpg'), cv2.cvtColor(input_data, cv2.COLOR_RGB2BGR))
    else:
        cv2.imwrite(os.path.join(high_dir,'both', 'image_'+str(i)+'.jpg'), cv2.cvtColor(input_data, cv2.COLOR_RGB2BGR))
        
for i in tqdm(range(len(mel_img_band))):
    input_data = mel_img_band[i][0]
    # print(input_data)
    labels = mel_img_band[i][1]
    # print(type(labels))
    
    if labels == 0: #1: abnormal, 0: normal
        cv2.imwrite(os.path.join(band_dir,'normal', 'image_'+str(i)+'.jpg'), cv2.cvtColor(input_data, cv2.COLOR_RGB2BGR))
    elif labels == 1:
        cv2.imwrite(os.path.join(band_dir,'crackle', 'image_'+str(i)+'.jpg'), cv2.cvtColor(input_data, cv2.COLOR_RGB2BGR))
    elif labels == 2:
        cv2.imwrite(os.path.join(band_dir,'wheeze', 'image_'+str(i)+'.jpg'), cv2.cvtColor(input_data, cv2.COLOR_RGB2BGR))
    else:
        cv2.imwrite(os.path.join(band_dir,'both', 'image_'+str(i)+'.jpg'), cv2.cvtColor(input_data, cv2.COLOR_RGB2BGR))
