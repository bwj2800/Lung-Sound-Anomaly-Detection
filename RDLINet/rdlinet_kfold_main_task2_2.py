...
image_dir = './Aug/Task2_2'
# image_dir = './Dataset_ICBHI_Log-Melspec/Dataset_Task_1/Dataset_1_2'
model_save_path = './checkpoint/rdlinet_kfold_task2_2.pth'
...
def train_and_evaluate():
    ...
    for fold, (train_index, test_index) in enumerate(skf.split(X, y)):
        ...
        # S_c=(S_p+S_e)/2
        s_healthy = avg_cm[0, 0] / avg_cm[0].sum()
        s_chronic = avg_cm[1, 1] / avg_cm[1].sum()
        s_nonchronic = avg_cm[2, 2] / avg_cm[2].sum()

        S_e = (s_chronic + s_nonchronic) / 2  # Sensitivity for Chronic and Non-Chronic
        S_p = s_healthy  # Specificity for Healthy
        S_c = (S_p + S_e) / 2

        fold_metrics.append([accuracy, s_healthy, s_chronic, s_nonchronic, S_e, S_p, S_c, f1])
        print(f'Accuracy on test set for fold {fold+1}: {accuracy:.2f}%')
        print(f'Healthy Accuracy: {s_healthy:.2%}')
        print(f'Chronic Accuracy: {s_chronic:.2%}')
        print(f'Non-Chronic Accuracy: {s_nonchronic:.2%}')
    ...
    print(f"Average Healthy Sensitivity: {mean_metrics[1]:.4f}")
    print(f"Average Chronic Sensitivity: {mean_metrics[2]:.4f}")
    print(f"Average Non-Chronic Sensitivity: {mean_metrics[3]:.4f}")
