...
model_save_path = './checkpoint/cnn_lstm_5fold.pth'
...
def train_and_evaluate():
    ...
    for fold, (train_index, test_index) in enumerate(skf.split(X, y)):
        ...
        train_indices, val_indices = next(StratifiedKFold(n_splits=5, shuffle=True, random_state=seed).split(train_index, y[train_index]))
        
        val_dataset = Subset(dataset, np.array(train_index)[val_indices])
        train_dataset = Subset(dataset, np.array(train_index)[train_indices])
        ...
        print(f'Wheeze Accuracy: {s_wheeze:.2%}')
        print(f'Both Accuracy: {s_both:.2%}')
        print(f"S_p: {S_p:.2f}, S_e: {S_e:.2f}, Score: {S_c:.2f}")
