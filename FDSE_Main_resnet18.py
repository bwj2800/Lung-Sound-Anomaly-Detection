...
def build_resnet18_model(input_shape, n_classes):
    inputs = keras.Input(shape=input_shape)
    x = kl.Conv2D(64, (3, 3), padding='same', activation='relu')(inputs)
    x = kl.Conv2D(64, (3, 3), padding='same')(x)
    x = kl.Add()([x, inputs])
    x = kl.Activation('relu')(x)
    x = kl.MaxPooling2D((2, 2))(x)

    x = kl.Conv2D(128, (3, 3), padding='same', activation='relu')(x)
    x = kl.Conv2D(128, (3, 3), padding='same')(x)
    x = kl.Add()([x, kl.Conv2D(128, (3, 3), padding='same', activation='relu')(inputs)])
    x = kl.Activation('relu')(x)
    x = kl.MaxPooling2D((2, 2))(x)

    x = kl.Conv2D(256, (3, 3), padding='same', activation='relu')(x)
    x = kl.Conv2D(256, (3, 3), padding='same')(x)
    x = kl.Add()([x, kl.Conv2D(256, (3, 3), padding='same', activation='relu')(inputs)])
    x = kl.Activation('relu')(x)
    x = kl.MaxPooling2D((2, 2))(x)

    x = kl.Conv2D(512, (3, 3), padding='same', activation='relu')(x)
    x = kl.Conv2D(512, (3, 3), padding='same')(x)
    x = kl.Add()([x, kl.Conv2D(512, (3, 3), padding='same', activation='relu')(inputs)])
    x = kl.Activation('relu')(x)
    ...
    x = kl.BatchNormalization()(x)
    x = kl.Dropout(0.5)(x)
    outputs = kl.Dense(n_classes, activation='softmax')(x)
...
model = build_resnet18_model(input_shape=X_train.shape[1:], n_classes=4)
